...
**ClipFlow는 클립보드 내용 등 개인정보를 수집, 전송 또는 공유하지 않습니다.** 모든 클립보드 데이터는 사용자의 기기에만 저장됩니다. 앱 안정성 개선을 위해 익명의 오류 보고만 수집합니다.
...
앱 안정성 개선을 위해 ClipFlow는 **Sentry** (Functional Software, Inc.)를 사용하여 자동 오류 보고를 수행합니다. 예기치 않은 오류 발생 시 다음의 **익명, 비개인** 데이터가 전송됩니다:
...
| 진단 로그 | `%LocalAppData%\ClipFlow\Logs\` | 로컬 문제 해결 |
| 오류 보고 캐시 | `%LocalAppData%\ClipFlow\sentry-cache\` | 오프라인 오류 보고 버퍼 |
